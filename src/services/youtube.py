...
async def download_audio(
    ...
    Raises:
        YouTubeError: If the video ID cannot be extracted or download fails
    """
    ...
    try:
        ...
        raise YouTubeError(f"Failed to download audio: {e}") from e

    if not os.path.exists(output_path):
        raise YouTubeError(f"Downloaded file not found: {output_path}")
...
def _download_with_ytdlp(
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        if info is None:
            raise YouTubeError("Failed to extract video info")
